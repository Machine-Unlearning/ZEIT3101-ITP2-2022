...
class Sensor(Commands):
    ...
    def __collision_detected_notify_helper(listener, packet):
        #print(packet)
        #unpacked = struct.unpack('>hhh?hhbL', packet.data) not sure how to interpret the data - so just not
        ...
        listener(CollisionDetected(0, 0, 0, 1, 0, 0, 0, 0, 0, 1))
    ...
    def start_robot_to_robot_infrared_following(toy, far_code, near_code, proc=None):
        packet = Sensor._encode(toy, 40, proc, [far_code, near_code])
        toy._execute(packet)
    ...
    def listen_for_robot_to_robot_infrared_message(toy, s, j, proc=None):  # Untested / Unknown param names
        toy._execute(Sensor._encode(toy, 43, proc, [s, s, s, s, s]))

    @staticmethod
    def __ir_detected_notify_helper(listener, packet):
        print(packet)
        listener(BotToBotInfraredReadings(1, 2, 4, 5))

    robot_to_robot_infrared_message_received_notify = (24, 44, 0xff), __ir_detected_notify_helper.__func__
